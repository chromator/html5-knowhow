	...
	function readQuizzes() {
		...
		db.transaction(function(tx) {
		tx.executeSql("SELECT quizId FROM Quiz;"
					, [], function(tx, result) {
			for (var i = 0, item = null; i < result.rows.length; i++) {
				item = result.rows.item(i);
				quizzes[i] = item['quizId'];
			}
		})
		});
		return quizzes;
	...
	function saveUser(username, password, mail) {
	...
	function verifyUser(username, successCallback, failureCallback) {
		var password;
		db.transaction(function(tx) {
		tx.executeSql("SELECT password FROM User WHERE username=?;"
					, [username], function(tx, result) {
