	...
	function initDb() {
		...
		// creates user table
		db.transaction(function(tx) {
			tx.executeSql("CREATE TABLE IF NOT EXISTS User(userId INTEGER PRIMARY KEY, username TEXT, password TEXT, emailId TEXT)", [],
	...
	function readQuestions(diffLevel, quizId) {
	...
	function saveUser(username, password, emailId) {
	...
	function verifyUser(emailId, successCallback, failureCallback) {
		var password;
		db.transaction(function(tx) {
		tx.executeSql("SELECT password FROM User WHERE emailId=?;"
					, [emailId], function(tx, result) {
